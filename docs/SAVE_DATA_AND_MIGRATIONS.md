...
- Serialize readers/writers with a narrow local lock. Reject a writer when the primary fingerprint changed since that repository instance loaded or wrote it; never silently apply a stale in-memory state.
- Reject saves above 16 MiB, malformed/non-integral versions, invalid envelope types, invalid active-record shapes, and malformed pet-domain collection fields before simulation access.
...
- Snapshot equality uses the canonical registry fingerprint, avoiding false mismatches from JSON integer/float representation changes.
...
`PetApplication` snapshots pet, foundation-save, offline-result and command-result state around mutating operations. Failed commands, simulation steps, offline synchronization or persistence restore that transaction snapshot. Presentation action timers also carry a revision so an older completion cannot overwrite a newer animation state.
