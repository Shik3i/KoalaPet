...
func _test_real_pack_discovery_and_snapshot() -> void:
	...
	_assert_equal(_example_registry.list_documents_by_kind("feature-gate").size(), 1, "CONTENT-003 feature-gate document indexed")
...
func _test_save_migration_fixture() -> void:
	...
	_assert_equal(loaded.data.save_format_version, 2, "SAVE-012 migration reaches current version")
