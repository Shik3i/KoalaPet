...
Prompt 4.6 adds three family walk atlases without changing the accepted visual language. Prompt 4.7 extends the same nine playable forms to a shared 29-animation contract with chronological `4–8`-frame idle, care, sleep, condition and battle sequences; current enemies receive attack/hit/dodge/defeat reactions. Moss/Ember/Tide effects stay separate from character textures and remain restrained at desktop-companion scale. All generated/derived material remains `PROVISIONAL_PRODUCT_REVIEW` with rights/license `UNDECIDED`.

Prompt 4.8 preserves that direction and rejects helper-hand/source-cell contamination from high-value care, call and sleep transitions. The complete visual state is reviewable through the runtime-backed debug Showroom and [`evidence/visual-acceptance/README.md`](evidence/visual-acceptance/README.md). Passing review means coherent provisional product-review art, not final art or rights approval.
